...
export const exhibits: ExhibitMeta[] = [
  {
    ...
    accuracyNotes: [
      ...
      'Once the matrix settles MU/TH/UR waits: arrow keys walk the white selection rule down both address columns and ENTER opens the record — INTERFACE 2037 is the only address wired to anything, as in the film. ENTER on a dead address gets a refusal — the rule blinks, the tick stutters. Ripley never picks wrong, so the film shows no such answer; the restoration invents one, because silence reads as a hang.',
      ...
      "MU/TH/UR answered to a keyboard in a 4:3 tube, and desktop keeps the keyboard — arrows, ENTER, nothing to click. Touch screens get the necessary heresies: the whole exhibit condenses into a 4:3 tube floating mid-screen, a tap walks the selection rule (a second tap on INTERFACE 2037 opens it), and your device's own keyboard stands in for the Nostromo's — tap the tube to raise it.",
    ],
  },
  {
    id: 'gullfire-nav',
    ...
    status: 'online',
    sound: true,
    component: lazy(() => import('./gullfire-nav')),
    accuracyNotes: [
      "The film's 'computer graphics' were a physical miniature of Manhattan painted black and dressed in reflective tape under ultraviolet light. This restoration inverts the trick: the island below is real survey data — OpenStreetMap building footprints and roof heights, New York's own building records — extruded into black masses whose taped edges alone return light.",
      "The data is today's Manhattan, with one restoration: the modern World Trade Center site is cleared and the Twin Towers reinstated on their original footprints at full height. The Gullfire needs its landing pad.",
      'The approach is flown live, not played back: altitude and airspeed on the HUD read from the flight itself, and the arrow keys slip the glide line sideways — the film shows Snake riding a fixed take, but a working nav display ought to answer its pilot.',
      "The approach flies the fiction's geometry: Snake launches from Liberty Island Security Control, so the Gullfire comes in over black harbor water from the southwest, towers dead ahead. The film's own nose-view footage cheats a long crawl over dense city blocks that no Liberty Island launch could produce — the restoration sides with the map.",
      "The 1981 shoreline bristled with finger piers the model dressed lovingly; today's waterfront keeps only a fraction of them, and the survey shows what stands now.",
      "The overlay's character generator is unidentified; DSEG14 Classic — an OFL segmented-display face — stands in for its blocky video type.",
      "The sound under the approach is the film's own — nine seconds of the glide's drone and wind, cut from the sequence and crossfaded into a seamless loop. The film's bed swells as the island nears; the loop holds one steady breath of it.",
      'The film shot a color video monitor; curvature, fringe and flicker are the shared CRT pass on its color-tube preset, tuned against the reference frames.',
    ],
  },
  {
    id: 'bio-helmet',
