...
const LOOK_AHEAD = DIR.clone()
  .multiplyScalar(600)
  .add(new Vector3(0, -280, 0));
...
function City() {
  ...
  const { tapeWidth } = useControls("city", {
    tapeWidth: { value: 2.1, min: 0.2, max: 4, step: 0.05 },
  });
...
function Flight() {
  ...
  useFrame(({ clock }, delta) => {
    ...
    look.current.lerp(
      TOWER_ROOF,
      MathUtils.smoothstep(t, FLARE_START, FLARE_FULL),
    );
    // the nose leads the slip a touch, so corrections read as banking
    look.current.addScaledVector(
      SIDE,
      (slipTarget.current - slip.current) * 0.4,
    );
    ...
    if (airRef.current) {
      const ms = Math.round(
        SPEED_MS + Math.sin(clock.getElapsedTime() * 1.3) * 1.5,
      );
      const next = `AIR..${ms}`;
